...
<body>
            ...
            <form action="" id="role_form" method="post" class="layui-form layui-form-pane" >
               ...
               <div class="layui-form-item">
                ...
                <table width="100%"  cellspacing="0" cellpadding="0" class="layui-table">
                    ...
                    <td>
                        <input type="text" id="name" name="name" value="{{ $good->name }}"   lay-verify="required"
                    ...
                    <td>
                        <input type="text" id="intro" name="intro" value="{{ $good->intro }}"    lay-verify="required"
                        autocomplete="off" class="layui-input">
                    </td>
                    </tr>  
                    ...
                    <td>原价</td>
                    <td>
                        <input type="text" id="original_price" name="original_price" value="{{ $good->original_price }}"
                        autocomplete="off" class="layui-input">
                    </td>
                    </tr>  
                    ...
                    <td>库存</td>
                    <td>
                        <input type="text" id="stock" name="stock"  value="{{ $good->stock }}"
                        autocomplete="off" class="layui-input">
                    </td>
                    </tr>  
                    ...
                    <td>编号</td>
                    <td>
                        <input type="text" id="sku" name="sku" value="{{ $good->sku }}"
                        autocomplete="off" class="layui-input">
                    </td>
                    </tr>  
                  ...
                  @if ($good->specifications)

                         @foreach ($good->specifications as $item)


                      
                            <tr class="specifications_tr">
                            <td><input type="text"  name="specifications_name[]" value="{{ $item['name'] }}" autocomplete="off" class="layui-input"></td>
                            <td><input type="text" name="specifications_val[]" value="{{ $item['value'] }}" autocomplete="off" class="layui-input"></td>
                  ...
                  @endif
                    ...
                    <td>
                    <button  type="button" class="layui-btn lay_up" data-id="{{ $good->good_id }}">
		                <i class="layui-icon">&#xe67c;</i>上传
		                  </button>

    <blockquote class="layui-elem-quote layui-quote-nm" style="margin-top: 11px;">
    <div class="layui-upload-list" id="{{ $good->good_id }}_upload-dan-preview">

      <li class="pre-li" data-image="{{ $good->image }}" ><input type="hidden" name="image" value="{{ $good->image }}" ><i class="icon iconfont">&#xe6b7;</i><img class="pre-img" src="/storage/{{ $good->image }}"  ></li>
    </div>
 </blockquote>
    ...
    <div class="layui-upload-list" id="upload-demo-preview">
      @foreach ( $good->images as $v)
        <li class="pre-li" data-image="{{ $v }}" ><input type="hidden" name="images[]" value="{{ $v }}" ><i class="icon iconfont">&#xe6b7;</i><img class="pre-img" src="/storage/{{ $v }}"  ></li>
      @endforeach
                    ...
                    @foreach ($cate_list as $item )
                          <input type="checkbox" name="cate_id[]" lay-skin="primary" {{ in_array($item['cate_id'],$good->cate_id_list())?'checked':'' }} value="{{ $item['cate_id'] }}"  title="{{ $item['name'] }}" >
                    @endforeach
                      </div>
                      </td>
                    </tr>
                    ...
                    @foreach ($brand_list as $item )
                          <input type="radio" name="brand_id" {{ $item['brand_id']==$good->brand_id?'checked':'' }} lay-skin="primary" value="{{ $item['brand_id'] }}"  title="{{ $item['name'] }}" >
                    @endforeach
                      </div>
                      </td>
                    </tr>
